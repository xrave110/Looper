...
contract fixedVault is ReentrancyGuard, VaultNFTv5 {
  ...
  modifier onlyRouter() {
    require(router == address(0) || msg.sender == router, "must use router");
    _;
  }
  ...
  function calculateCollateralProperties(uint256 _collateral, uint256 _debt) private view returns (uint256, uint256) {
    ...
    uint256 collateralValue = _collateral * getEthPriceSource() * decimalDifferenceRaisedToTen;
  ...
  function isValidCollateral(uint256 _collateral, uint256 debt) public view returns (bool) {
  ...
  function calculateFee(uint256 fee, uint256 amount, uint256 promoFee) public view returns (uint256) {
    uint256 _fee;
    if (promoFee > 0) {
      _fee = ((amount * fee * getTokenPriceSource() * promoFee) / (getEthPriceSource() * TEN_THOUSAND * TEN_THOUSAND));
    } else {
      _fee = (amount * fee * getTokenPriceSource()) / (getEthPriceSource() * TEN_THOUSAND);
    }
  ...
  function depositCollateral(uint256 vaultID, uint256 amount) external vaultExists(vaultID) onlyRouter {
  ...
  function withdrawCollateral(uint256 vaultID, uint256 amount) external onlyVaultOwner(vaultID) nonReentrant {
    ...
    if (debt != 0) {
      require(isValidCollateral(newCollateral, debt), "Withdrawal would put vault below minimum collateral percentage");
    }
  ...
  function borrowToken(
    ...
    uint256 amount,
    uint256 _front
  ) external frontExists(_front) onlyVaultOwner(vaultID) nonReentrant {
    ...
    require(((vaultDebt(vaultID)) + amount) >= minDebt, "Vault debt can't be under minDebt");
  ...
  function paybackTokenAll(
    ...
    require(deadline >= block.timestamp, "paybackTokenAll: deadline expired.");
  ...
  function payBackToken(
    ...
    require(vaultDebtNow >= amount, "Vault debt less than amount to pay back");

    require(((vaultDebtNow) - amount) >= minDebt || amount == (vaultDebtNow), "Vault debt can't be under minDebt");

    uint256 _closingFee = calculateFee(closingFee, amount, promoter[_front]);
  ...
  function checkCost(uint256 vaultID) public view returns (uint256) {
    ...
    if (vaultCollateral[vaultID] == 0 || vaultDebtNow == 0 || !checkLiquidation(vaultID)) {
      return 0;
    }
  ...
  function checkCollateralPercentage(uint256 vaultID) public view vaultExists(vaultID) returns (uint256) {
    uint256 vaultDebtNow = vaultDebt(vaultID);
  ...
  function liquidateVault(uint256 vaultID, uint256 _front) external frontExists(_front) vaultExists(vaultID) {
  ...
  function buyRiskDebtVault(uint256 vaultID) external vaultExists(vaultID) returns (uint256) {
    require(stabilityPool == address(0) || msg.sender == stabilityPool, "buy risky is disabled for public");
    uint256 vaultDebtNow = updateVaultDebt(vaultID);
    ...
    uint256 collateralPercentage = collateralValueTimes100 / (debtValue);
    require((collateralPercentage * 10) <= gainRatio, "Vault is not below risky collateral percentage");

    uint256 maiDebtTobePaid = (debtValue / (10 ** priceSourceDecimals)) -
      (collateralValueTimes100 / (_minimumCollateralPercentage * (10 ** priceSourceDecimals)));
...
contract stableQiVault is fixedVault, Ownable {
  ...
  constructor(
    ...
    address _collateral,
    string memory baseURI
  ) fixedVault(ethPriceSourceAddress, minimumCollateralPercentage, name, symbol, _mai, _collateral, baseURI) {
    createVault();
    addFrontEnd(0);
  }
  ...
  /// @dev dust protection
  function setMinDebt(uint256 _minDebt) external onlyOwner {
  ...
  function updateFrontEnd(
    uint256 _promoter,
    uint256 cashback
  ) external frontExists(_promoter) onlyVaultOwner(_promoter) {
