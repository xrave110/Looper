...
contract Comptroller is ComptrollerV7Storage, ComptrollerErrorReporter, ExponentialNoError {
  ...
  event NewCollateralFactor(IOToken oToken, uint256 oldCollateralFactorMantissa, uint256 newCollateralFactorMantissa);
  ...
  event ContributorRewardSpeedUpdated(address indexed contributor, uint256 newSpeed);
   ...
   */
  function enterMarkets(address[] memory oTokens) public override returns (uint256[] memory) {
  ...
  function exitMarket(address oTokenAddress) external override returns (uint256) {
    ...
    (uint256 oErr, uint256 tokensHeld, uint256 amountOwed, ) = oToken.getAccountSnapshot(msg.sender);
    ...
    uint256 allowed = redeemAllowedInternal(oTokenAddress, msg.sender, tokensHeld);
    if (allowed != 0) {
   ...
   */
  function mintAllowed(address oToken, address minter, uint256 mintAmount) external override returns (uint256) {
   ...
   */
  function mintVerify(address oToken, address minter, uint256 actualMintAmount, uint256 mintTokens) external override {
   ...
   */
  function redeemAllowed(address oToken, address redeemer, uint256 redeemTokens) external override returns (uint256) {
   ...
   */
  function borrowAllowed(address oToken, address borrower, uint256 borrowAmount) external override returns (uint256) {
    // Pausing is a very serious situation - we revert to sound the alarms
   ...
   */
  function borrowVerify(address oToken, address borrower, uint256 borrowAmount) external override {
    // Shh - currently unused
    oToken;
    borrower;
    borrowAmount;
  ...
  function liquidateBorrowAllowed(
    ...
    uint256 borrowBalance = IOToken(oTokenBorrowed).borrowBalanceStored(borrower);
    ...
    if (isDeprecated(IOToken(oTokenBorrowed))) {
      ...
      (Error err, , uint256 shortfall) = getHypotheticalAccountLiquidityInternal(borrower, IOToken(address(0)), 0, 0);
   ...
   * @param transferTokens The number of oTokens to transfer
   */
  function transferVerify(address oToken, address src, address dst, uint256 transferTokens) external override {
    // Shh - currently unused
    oToken;
    src;
    dst;
  ...
  function liquidateCalculateSeizeTokens(
    ...
    uint256 priceBorrowedMantissa = oracle.getUnderlyingPrice(IOToken(oTokenBorrowed));
    uint256 priceCollateralMantissa = oracle.getUnderlyingPrice(IOToken(oTokenCollateral));
     ...
     */
    uint256 exchangeRateMantissa = IOToken(oTokenCollateral).exchangeRateStored(); // Note: reverts on error
   ...
   */
  function _setCollateralFactor(IOToken oToken, uint256 newCollateralFactorMantissa) external returns (uint256) {
    // Check caller is admin
    if (msg.sender != admin) {
  ...
  function _supportMarket(IOToken oToken, bool _autoCollaterize) external returns (uint256) {
    ...
    markets[address(oToken)] = Market({isListed: true, autoCollaterize: _autoCollaterize, collateralFactorMantissa: 0});
  ...
  function _setMarketBorrowCaps(IOToken[] calldata oTokens, uint256[] calldata newBorrowCaps) external {
    require(msg.sender == admin || msg.sender == borrowCapGuardian, "only admin or borrowCapGuardian");
  ...
  function onlyAdminOrGuardian() internal view {
    require(msg.sender == admin || msg.sender == pauseGuardian, "only pause guardian and admin");
  }

  function _setMintPaused(IOToken oToken, bool state) public returns (bool) {
    require(markets[address(oToken)].isListed, "cannot pause: market not listed");
    onlyAdminOrGuardian();
  ...
  function _become(IUnitroller unitroller) public {
    require(msg.sender == unitroller.admin(), "only unitroller admin can _become");
   ...
   */
  function setRewardSpeedInternal(IOToken oToken, uint256 supplySpeed, uint256 borrowSpeed) internal {
  ...
  function updateRewardSupplyIndex(address oToken) internal {
    ...
    if (deltaBlocks > 0) {
      if (supplySpeed > 0) {
        ...
        supplyState.index = safe224(add_(Double({mantissa: supplyState.index}), ratio).mantissa);
      }
   ...
   * @dev Index is a cumulative sum of the Reward per oToken accrued.
   */
  function updateRewardBorrowIndex(address oToken, Exp memory marketBorrowIndex) internal {
    MarketState storage borrowState = borrowState[oToken];
    ...
    if (deltaBlocks > 0) {
      if (borrowSpeed > 0) {
        ...
        borrowState.index = safe224(add_(Double({mantissa: borrowState.index}), ratio).mantissa);
      }
   ...
   */
  function distributeSupplierReward(address oToken, address supplier) internal {
    ...
    emit DistributedSupplierReward(IOToken(oToken), supplier, supplierDelta, supplyIndex);
  }
   ...
   */
  function distributeBorrowerReward(address oToken, address borrower, Exp memory marketBorrowIndex) internal {
    ...
    emit DistributedBorrowerReward(IOToken(oToken), borrower, borrowerDelta, borrowIndex);
  }
   ...
   */
  function claimRewards(address[] memory holders, IOToken[] memory oTokens, bool borrowers, bool suppliers) public {
    for (uint256 i = 0; i < oTokens.length; i++) {
      ...
      if (borrowers) {
        ...
        for (uint256 j = 0; j < holders.length; j++) {
          distributeBorrowerReward(address(oToken), holders[j], borrowIndex);
        }
      }
      if (suppliers) {
        updateRewardSupplyIndex(address(oToken));
  ...
  function _setRewardSpeeds(
    ...
    for (uint256 i = 0; i < numTokens; ++i) {
      setRewardSpeedInternal(IOToken(oTokens[i]), supplySpeeds[i], borrowSpeeds[i]);
    }
  }
   ...
   */
  function _setContributorRewardSpeed(address contributor, uint256 rewardSpeed) public {
    require(msg.sender == admin || msg.sender == rewardUpdater, "only admin can set reward speed");
